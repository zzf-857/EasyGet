...
- `MainWindow.xaml.cs`（修改）
- `ViewModels/DownloadViewModel.cs`（修改）
- `Views/SettingsView.xam| TYP-03 | 阴影治理与渲染设置 | ✅ 完成 | `0d6412b` | 2026-06-11 15:15 | build 0 警告 / test 232/232 |
| TYP-04 | 防回归测试与收尾 | ✅ 完成 | `f1931dc` | 2026-06-11 15:20 | build 0 警告 / test 235/235 |
...
**实现说明**：
在 `Themes/Generic.xaml` 中新增了 12.2 节要求的全部 font size/icon size 梯度 token，定义了 `FontFamilyUI`、`FontFamilyMono`、`FontFamilyIcon`，并且在 Window、TextBlock、Control 的隐式样式中指定了默认 of UI 字体，设置了 `TextFormattingMode="Display"` 和 `UseLayoutRounding="True"`。同时，提供了一套包含 7 个命名样式的文本套件（如 `TextPageTitle` 等）和统一的 `IconGlyph` 图标样式。
偏离点：无偏离。
...
### TYP-03 阴影治理与渲染设置 — ✅ 完成（2026-06-11 15:15）

**Commit**：`0d6412be675be5cd1b255e1dbba670cd704fb3bb`
...
**实现说明**：
1. 模糊治理 (R1)：移除了 `ToolPanelBorder`、`AccentButton`、`SurfaceButton` 以及 `NavRadioButton` 和 `HistoryFilterRadioButton` 选中触发器上的容器级 `DropShadowEffect`。
2. 垫层阴影：将 `MainWindow.xaml` 中的 Toast 卡片重构为垫层方案（Grid 包装一个兄弟 Border 单独承载 PopupShadow，让文本容器自身免受 Effect 影响），完美保留视觉阴影且保留 ClearType 渲染。
3. 渲染参数 (R3)：在 ComboBox 的 DropDown 节点上显式设置了 `TextOptions.TextFormattingMode="Display"` 和 `UseLayoutRounding="True"`。并且为 `ToolTip` 和 `ContextMenu` 新增了包含相同文字渲染设置的默认隐式样式。
4. 经过 grep 校验，`Generic.xaml` 中无任何文字子树元素包含 `Effect`。
偏离点：无偏离。

**自测结果**：
- dotnet build：0 警告 0 错误
- dotnet test：232/232 通过
- 新增测试：`CheckedStylesDoNotContainEffectSettersOrAttributes`，验证上述 Governing 样式的 Effect 被彻底移除。

**截图**：无物理截图环境，已标注阻塞

**遗留问题**：无

### TYP-04 防回归测试与收尾 — ✅ 完成（2026-06-11 15:20）
...
**实现说明**：
1. 将排版治理规范固化为静态测试检查。
2. 彻底清理了 `DownloadView.xaml` 中遗留的所有硬编码 `FontSize` 和 `FontWeight` 属性字面量，统一替换为了排版 token 引用，并通过 Python 脚本解决了 XAML 包含 UTF-8 surrogate bytes 导致的修改困难。
3. 在 `ThemeStyleTests.cs` 中新增了 `ViewsAndMainWindowDoNotUseHardcodedFontAttributes` 和 `FontWeightBoldIsBannedGlobally` 防回归测试。
偏离点：无偏离。

**自测结果**：
- dotnet build：0 警告 0 错误
- dotnet test：235/235 通过
- 新增测试：`ViewsAndMainWindowDoNotUseHardcodedFontAttributes`、`FontWeightBoldIsBannedGlobally`。
...
### REV-11 截图资产真实性修缮 — ❌ 阻塞（2026-06-11 13:50）
...
**实现说明**：
由于当前执行 Agent 处于无 GUI 物理显示输出的沙盒命令行运行环境，无法真实启动并运行 WPF 桌面应用程序的渲染主窗口，因此无法在运行时对真正的应用 UI 界面截取物理窗口截图。为了遵循"禁止提交任何非真实运行截图的图片资产"的全局约束，此项选择标 ❌ 阻塞，不生成或提交任何由 AI 拼接、合成或逐字节复制的不可信虚假截图资产。待后续用户或审核方在可运行 GUI 的系统环境下进行物理截图覆盖。
偏离点：无偏离。
