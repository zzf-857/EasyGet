...
EasyGet 是一款基于 [yt-dlp](https://github.com/yt-dlp/yt-dlp) 并扩展专用解析器的 Windows 桌面视频下载工具，支持 YouTube、Bilibili、X(Twitter)、Instagram、抖音、央视频等网站的视频下载。项目采用 WPF + MVVM 架构，提供暗色主题、多任务队列、下载历史、智能登录与 Cookie、播放列表导入和环境自动安装等能力。

## 🖼️ 界面设计存档（v1.0）

以下图片是 2026-06-11 的 v1.0 设计与过程存档，部分素材为概念图，仅作历史参考，不代表当前版本的实机界面。

| 视频下载 | 批量下载 |
|---|---|
| ![视频下载页](Designer/v1.0/2026-06-11-readme-screenshots/download-view.png) | ![批量下载页](Designer/v1.0/2026-06-11-readme-screenshots/batch-download-view.png) |

| 历史记录 | 设置 |
|---|---|
| ![历史记录页](Designer/v1.0/2026-06-11-readme-screenshots/history-view.png) | ![设置页](Designer/v1.0/2026-06-11-readme-screenshots/settings-view.png) |

## ✨ 功能特性

### 🎯 核心功能
- **单视频下载** — 输入 URL 后自动解析标题、平台、时长、缩略图，支持格式、画质、字幕与目录选择
- **批量下载** — 多 URL 去重后立即全部入队，后台有界并发解析元数据，同平台任务共享认证结果；支持播放列表导入、总进度/完成数量/总速度/ETA 展示、队列筛选和批量控制
- **智能登录与 Cookie** — 公开内容先匿名下载；确需登录时自动尝试本机浏览器配置，全部不可用才显示一次平台托管登录窗口，登录后原任务自动继续
- **任务控制** — 支持单项停止、暂停、恢复、失败或取消后重试、停止全部未完成任务与清理已结束任务
...
- aria2c 外部下载器开关
- 智能 Cookie 默认开启，支持 Chrome、Edge、Firefox、Brave、Chromium、Vivaldi、Opera 和 Opera GX 的多个配置
- 高级手动 Cookie 导入必须绑定平台；Header、JSON 和 Netscape 格式按目标域隔离并使用 Windows DPAPI 加密保存
- 旧版本遗留但尚未归属平台的 Header Cookie 会先进入 DPAPI 加密隔离区，重新打开应用后仍可选择平台完成迁移，不会在退出时静默丢失
- 关闭智能 Cookie 自动探测后，用户主动保存的加密手动 Cookie 仍可使用；可从高级区域按平台独立清除
- 下载完成后按平台自动归类保存
- JSON 配置自动持久化
...
- **认证隐私边界** — 不读取或显示账号密码、验证码和 Cookie 值；日志、下载历史、普通配置备份不记录 Cookie 内容或完整浏览器配置路径
- **全局异常捕获与崩溃日志**

### 🔐 智能登录如何工作

1. EasyGet 先匿名解析并下载公开内容，不弹登录窗口。
2. 只有遇到明确的登录、年龄、机器人验证或 Cookie 失效错误时，才按最近成功记录、默认浏览器和最近活动时间尝试本机浏览器配置。
3. 所有本机方案均不可用时，EasyGet 打开目标平台的真实登录页。账号、密码和验证码只提交给平台网页；登录完成后等待中的任务自动继续。
4. 同一平台的批量任务共享一次认证请求，不重复弹窗；不同平台可并行处理。

“智能登录”不会绕过平台登录、验证码、地区限制、付费权限或服务条款。用户从未登录、会话过期或平台要求验证时，仍需在平台官网完成一次正常登录。详细说明见 [Cookie 获取与使用操作手册](00Readme/cookie-获取与使用操作手册.md)。
...
| **Microsoft.Web.WebView2** | 平台托管登录窗口 |
| **yt-dlp** | 视频下载核心引擎 |
| **ffmpeg** | 音视频合并转码 |
| **xUnit** | 单元测试 |
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\build-installer.ps1 -Version 1.4.1
```
...
- [x] 批量任务总进度、完成数量、速度、ETA、状态筛选与批量控制
- [x] SQLite 历史资料库（搜索 / 合集折叠 / 自定义文件夹 / 多选 / 拖拽 / 批量管理）
...
- [x] 缩略图解析与展示
- [x] 平台隔离的智能 Cookie 编排、浏览器自动发现、WebView2 托管登录和加密手动导入
- [x] 按平台自动归类保存
- [x] 抖音浏览器兜底下载
...
- [x] 央视频公开视频页面识别、动态临时地址解析与 MP4 断点续传下载
...
- [x] 800+ 个稳定自动化测试（覆盖 Cookie 安全与迁移、批量并发、ViewModel、命令、XAML、m3u8、Telegram、央视频与发布更新链路；真实站点另走脱敏手工验收）
...
| 手动 Cookie 加密仓库 | `%LocalAppData%/EasyGet/manual-cookies/`（按平台隔离；未归属的旧版内容暂存为加密隔离项；均使用 Windows DPAPI） |
| EasyGet 托管登录会话 | `%LocalAppData%/EasyGet/sessions/<platform>/`（各平台独立 WebView2 用户目录） |
| Cookie 健康元数据 | `%LocalAppData%/EasyGet/cookie-health.json`（仅策略与成功/失败类别，不含 Cookie 值或完整配置路径） |
| Cookie 临时租约 | `%LocalAppData%/EasyGet/temp/cookies/`（仅当前 Windows 用户可访问，进程结束即删除，启动时清理崩溃遗留） |
