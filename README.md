...
EasyGet 是一款基于 [yt-dlp](https://github.com/yt-dlp/yt-dlp) 的 Windows 桌面视频下载工具，支持 YouTube、Bilibili、X(Twitter)、Instagram、抖音等 [1000+ 网站](https://github.com/yt-dlp/yt-dlp/blob/master/supportedsites.md) 的视频下载。项目采用 WPF + MVVM 架构，提供暗色主题、多任务队列、下载历史、智能登录与 Cookie、播放列表导入和环境自动安装等能力。
...
- **批量下载** — 多 URL 去重后立即全部入队，后台有界并发解析元数据，同平台任务共享认证结果；支持播放列表导入、队列管理、并发下载控制和单任务操作
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\build-installer.ps1 -Version 1.1.8
```
...
- [x] 700+ 个稳定自动化测试（覆盖 Cookie 安全与迁移、批量并发、ViewModel、命令、XAML、m3u8、Telegram 与发布更新链路；真实站点另走脱敏手工验收）
