...
2. **抖音浏览器兜底仍需异常场景硬化** — 已实现 Chrome/Edge headless 捕获 mp4 下载，但还需要覆盖超时、Range 续传、临时文件清理和无浏览器场景
3. **yt-dlp 长下载进程缺少卡住保护** — 环境命令和 yt-dlp 短命令已增加超时和 stderr 读取，但长时间下载仍需要无输出超时、诊断输出和更细粒度取消保护
